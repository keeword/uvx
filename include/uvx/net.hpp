...
inline ip4_addr to_ip4_addr(const std::string& ip, int port) {
    ...
    if (uv_ip4_addr(ip.c_str(), port, &addr) != 0) {
        throw std::runtime_error("can not convert ip address: " + ip);
    }
...
inline ip6_addr to_ip6_addr(const std::string& ip, int port) {
    ip6_addr addr;
    if (uv_ip6_addr(ip.c_str(), port, &addr) != 0) {
        throw std::runtime_error("can not convert ip address: " + ip);
    }
...
inline bool from_ip4_addr(const ip4_addr* src, std::string& ip, int& port) {
    char dest[16];
...
inline bool from_ip6_addr(const ip6_addr* src, std::string& ip, int& port) {
    char dest[46];
